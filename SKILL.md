...
Follow `setup.md` to interactively configure the test commands, test framework, editable test scope, and baseline.

### Subsequent runs (config exists)

Follow `loop.md` to run the test-writing loop. The agent reads the config, context note, and recent history, then enters the autonomous loop.
